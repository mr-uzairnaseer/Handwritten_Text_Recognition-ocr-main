...
export default function ImageViewer({ preview, onUploadClick }) {
    ...
    return (
        <div className="flex flex-col h-full bg-zinc-950/50">
            ...
            {/* Viewport */}
            <div className="flex-grow overflow-hidden relative flex items-center justify-center bg-[radial-gradient(#27272a_1px,transparent_1px)] [background-size:16px_16px]">
                {preview ? (
                    <div
                    ...
                    >
                        <img
                            src={preview}
                            alt="Source"
                            className="max-w-full max-h-[80vh] object-contain shadow-2xl rounded-sm ring-1 ring-white/10"
                        />
                    </div>
                ) : (
                    <div
                        onClick={onUploadClick}
                        className="group flex flex-col items-center justify-center p-12 border-2 border-dashed border-zinc-800 hover:border-blue-500/50 rounded-xl bg-zinc-900/20 hover:bg-zinc-900/50 transition-all cursor-pointer select-none"
                    >
                        <div className="w-20 h-20 rounded-full bg-zinc-900 group-hover:bg-blue-600/10 flex items-center justify-center ring-1 ring-zinc-800 group-hover:ring-blue-500/50 mb-6 transition-all shadow-lg group-hover:shadow-blue-900/20">
                            <ImageIcon size={40} className="text-zinc-600 group-hover:text-blue-400 transition-colors" />
                        </div>
                        <h3 className="text-lg font-bold text-zinc-300 group-hover:text-blue-200 transition-colors mb-2">
                            Upload Image
                        </h3>
