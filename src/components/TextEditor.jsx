    ...
    return (
        <div className="flex flex-col h-full bg-[#1e1e1e]">
            ...
            <div className="h-10 border-b border-zinc-800 bg-zinc-900/50 flex items-center justify-between px-4">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="flex items-center gap-1.5 px-2 py-1 rounded text-xs font-medium text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
            ...
            <div className="flex-grow overflow-hidden relative flex font-mono text-sm leading-6">
                {/* Line Numbers */}
                <div className="w-12 flex-shrink-0 bg-[#1e1e1e] border-r border-zinc-800 text-zinc-600 text-right pr-3 pt-4 select-none">
                ...
                {/* Content */}
                <div className="flex-grow overflow-auto pt-4 pl-4 text-zinc-300">
                    {displayContent ? (
...
function Tab({ icon, label, active, onClick }) {
    return (
        <div
            onClick={onClick}
            className={`
            flex items-center gap-2 h-10 px-4 border-r border-zinc-800 cursor-pointer select-none transition-colors
            ${active ? 'bg-[#1e1e1e] text-zinc-200 border-t-2 border-t-blue-500' : 'bg-transparent text-zinc-500 hover:bg-zinc-800/50 hover:text-zinc-300'}
        `}>
            {icon}
            <span className="text-xs font-medium">{label}</span>
            {active && <span className="ml-2 w-2 h-2 rounded-full bg-zinc-600/50 hover:bg-zinc-500"></span>}
        </div>
    );
}
